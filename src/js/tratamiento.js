...
async function resolveUsuarioId(usuarioString){
  ...
  try{
    const res = await fetch('http://52.70.236.29:7000/usuarios', { headers: await getAuthHeaders() });
...
async function fetchReportes(){
  try{
    ...
    const res = await fetch('http://52.70.236.29:7000/reportes', { headers: await getAuthHeaders() });
...
async function fetchMedicamentos(){
  try{
    ...
    const res = await fetch('http://52.70.236.29:7000/medicamento', { headers: await getAuthHeaders() });
...
async function updateTratamientoBackend(idTratamiento, payload, idUsuarioHeaderOverride){
  try{
    ...
    const res = await fetch(`http://52.70.236.29:7000/tratamientos/${idTratamiento}`, { method: 'PUT', headers, body: JSON.stringify(payload) });
...
async function deleteTratamientoBackend(idTratamiento){
  try{
    ...
    const res = await fetch(`http://52.70.236.29:7000/tratamientos/${idTratamiento}`, { method: 'DELETE', headers });
    const text = await res.text(); let body = text? ((()=>{ try{return JSON.parse(text);}catch(e){return text;} })()):null;
...
btnGuardar.addEventListener('click', () => {
  ...
  if(editIndex !== null){
    ...
    try{
      ...
      const res = await fetch('http://52.70.236.29:7000/tratamientos', { method: 'POST', headers, body: JSON.stringify(payload) });
      const text = await res.text(); let body = text? ((()=>{ try{return JSON.parse(text);}catch(e){return text;} })()):null;
